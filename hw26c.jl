...
#    x = final point
#
function SteepestDescent(x0,alpha,beta)
   beta=beta
   alpha=alpha
...
alphas=[.0000001 .000001 .00001 .0001 .001 .002]
betas=[.0001 .001 .002 .003 .004 .005 .006 .007 .008 .009 .01 .02 .03 .04 .05 .1 .5]

for i in alphas
    for j in betas
        print(SteepestDescent(x_null,i,j))
        print("-------")
    end
end